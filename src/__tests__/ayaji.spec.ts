...
const transform = (markdown: string, options?: Options): string => {
  const result = remark()
    .use(remarkParse)
...
describe(`remark-ayaji`, () => {
  it(`works`, () => {
    const actual = transform(`\`\`\`jp\n日本語\n\`\`\``);
    const expected = `<p><span class="noun">日本語</span></p>`;

    expect(actual).toStrictEqual(expected);
  });

  it(`prefixes should be classified as the word they are attached to`, () => {
    const actual = transform(`\`\`\`jp\nご高\n\`\`\``);
  ...
  describe(`supports local settings`, () => {
    it(`overrides global settings`, () => {
      const actual = transform(`\`\`\`jp furigana: false\n日本語\n\`\`\``, { furigana: true });
      const expected = `<p><span class="noun">日本語</span></p>`;

      expect(actual).toStrictEqual(expected);
    });

    it(`logs a message when an invalid local config is supplied`, () => {
      const consoleMock = vi.spyOn(console, `log`).mockImplementation(() => undefined);
      const actual = transform(`\`\`\`jp foo\n日本語\n\`\`\``);
      const expected = `<p><span class="noun">日本語</span></p>`;
    ...
    it(`furigana`, () => {
      const actual = transform(`\`\`\`jp furigana: true\n日本語\n\`\`\``);
      const expected = `<p><span class="noun">{日本語|にほんご}</span></p>`;
    ...
    it(`include`, () => {
      const actual = transform(`\`\`\`jp include: ["noun"]\n日本語が分かります\n\`\`\``);
      const expected = `<p><span class="noun">日本語</span>が分かります</p>`;
    ...
    it(`exclude`, () => {
      const actual = transform(`\`\`\`jp exclude: ["noun", "auxiliary-verb"]\n日本語が分かります\n\`\`\``);
      const expected = `<p>日本語<span class="particle">が</span><span class="verb">分かり</span>ます</p>`;
